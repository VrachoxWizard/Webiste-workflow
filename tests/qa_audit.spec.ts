...
test.describe("Pointershop Premium Redesign QA Audit", () => {
  ...
  test("2. Command Menu (Cmd+K) Interaction", async ({ page }) => {
    ...
    await expect(cmdMenu.getByPlaceholder(/Pretražite/i)).toBeVisible()

    await page.screenshot({ path: "test-results/search-command-palette.png" })

    // Search for a product
    await cmdMenu.getByPlaceholder(/Pretražite/i).fill("Beretta")
    await expect(cmdMenu.getByText(/Beretta 686 Silver Pigeon/i)).toBeVisible()
  ...
  test("5. Regulated Item Workflow (PDP -> Cart -> Checkout)", async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/proizvod/1`)
    ...
    await page.goto(`${BASE_URL}/proizvod/8`)
    ...
    await expect(cartDrawer.getByText(/Meindl Dovre Extreme GTX/i)).toBeVisible()
  ...
  test("6. Responsive Drawer & Category Navigation", async ({ page }) => {
    ...
    const mobileDrawer = page.getByRole("dialog", { name: "TerraLov" })
    await expect(mobileDrawer.getByText("Kategorije", { exact: true })).toBeVisible()
