    ...
    <div className="mx-auto flex h-screen max-w-md flex-col p-4">
      <div className="mb-4 flex-1 overflow-y-auto">
        {messages.map((m) => (
          <div key={m.id} className={`mb-4 ${m.role === "user" ? "text-right" : "text-left"}`}>
            <span
              className={`inline-block rounded-lg p-2 ${m.role === "user" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
            >
              {m.target || m.content}
            </span>
          </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          ...
          disabled={isLoading}
        />
        <button type="submit" disabled={isLoading} className="rounded bg-black p-2 text-white">
          Send
        </button>
      </form>
    </div>
